...
class TitleBlock {
	...
	public static function listCallback( $editPage, $text, $section, &$hookError ) {
		wfProfileIn( __METHOD__ );
	...
	public static function newWikiBuilder( &$api, $titleObj, $category, $text ) {
		wfProfileIn( __METHOD__ );
	...
	// used in Answers' CreateDefaultQuestionPage
	public static function genericTitleCheck( $titleObj ) {
		wfProfileIn( __METHOD__ );
		...
		if ( !empty( self::$blocksData ) ) {
			foreach ( self::$blocksData as $blockData ) {
				...
				if ( $result['blocked'] ) {
					...
					wfDebugLog( 'Phalanx', __METHOD__ . ": block '{$result['msg']}' blocked '$fullText'." );
					break;
				}
			}
		}
